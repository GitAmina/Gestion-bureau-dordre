...
const ChartThree: React.FC = () => {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      setSeries([data.enAttente || 0, data.Traite || 0, data.Cloture || 0]);
    } catch (err: any) {
  ...
  useEffect(() => {
    fetchStatistics();
    const interval = setInterval(fetchStatistics, 200000);
  ...
  const options: ApexOptions = {
    ...
    labels: ["En attente", "Traité", "Clôturé"],
        ...
        <>
          ...
          <div className="mx-auto w-full max-w-[350px]">
            <div className="-mx-7.5 flex flex-wrap items-center  gap-y-2.5">
              {["En attente", "Traité", "Clôturé"].map((label, index) => (
