...
const RapportsTable = () => {
  ...
  const handleGenerateReport = async () => {
    setLoading(true);
    try {
      const response = await axios.post("/api/rapports/generate", {
  ...
  const handleViewDetails = (report: Report) => {
    ...
    toast.success("Les informations du rapport ont été affichées !");
  };
  ...
  const handleDelete = async (rapportId: number) => {
    ...
    try {
      ...
      if (response.ok) {
        setReports((prevRapports) =>
          prevRapports.filter((r) => r.id !== rapportId),
        );
  ...
  return (
    <div>
        ...
        <div className="flex space-x-4">
          <DefaultSelectOption
            ...
            onChange={(value: string) =>
              setSelectedType(value === "Annuel" ? "Annuel" : "Mensuel")
            }
          />
          <DefaultSelectOption
            options={["PDF", "Excel"]}
            onChange={(value: string) =>
              setSelectedFormat(value === "Excel" ? "Excel" : "PDF")
            }
          />
          <button
      ...
      {/* Affichage des détails du rapport sélectionné */}
      {selectedReport && (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <p>
              ...
              <span className="font-medium text-gray-700 dark:text-gray-300">
                {selectedReport.date_generation
